...
public class Driver 
{
    public static void main(String[] args) 
    {
        ...
        do 
        {
            ...
            do 
            {
                ...
                StdIn.readLine(); System.err.println();
                
                switch (choice) 
                {
                    case 1:
                    testSortedList(input); break;
                    
                    case 2:
                    testMakeTree(input); break;
                    
                    case 3:
                    testMakeEncodings(input); break;
                    
                    case 4:
                    testEncode(input); break;
                    
                    case 5:
                    testDecode(input); break;
    ...
    private static void testSortedList(String input) 
    {
        ...
        StdOut.print(charToString(sortedList.get(0).getCharacter()) + ": " + sortedList.get(0).getProbOcc() + "      ");

        for (int i = 1; i < sortedList.size(); i++) 
        {
            char c = sortedList.get(i).getCharacter();
            
            StdOut.print(charToString(c)+ ": " + sortedList.get(i).getProbOcc() + "    ");
        }
    ...
    private static void testMakeTree(String input) 
    {
        ...
        HuffmanCoding studentSolution = new HuffmanCoding(input);
        studentSolution.makeSortedList(); studentSolution.makeTree();
        
        StdOut.println("Note that the decimals are rounded to 2 decimal places\n");
        printTree(studentSolution.getHuffmanRoot()); StdOut.println();
    }
    ...
    private static void printTree(TreeNode n, String indent, boolean isRight, boolean isRoot) 
    {
        ...
        //StdOut.printf("%.2f ", n.getData().getProbOcc()); StdOut.println();
        StdOut.print(n.getData().getProbOcc()); StdOut.println();
        ...
        printTree(n.getRight(), indent, true, false); printTree(n.getLeft(),  indent, false, false);
    }
    ...
    private static void testMakeEncodings(String input) 
    {
        ...
        for (int i = 0; i < 128; i++) 
        {
            if (encodings[i] != null) 
            {
                ...
                //StdOut.printf("%s->%s", charToString((char)i), encodings[i]); first = false;
                StdOut.print("Index" + i + " has encoding of " + encodings[i]); StdOut.println();
            }
        }
    ...
    private static void testEncode(String input) 
    {
        ...
        String encodedFile = StdIn.readLine(); System.err.println();
